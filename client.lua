...
RegisterNetEvent('doom-spawn:client:openUI', function(value)
...
RegisterNetEvent('doom-spawn:client:setupSpawns', function(cData, new, apps)
    if not new then
        local PlayerData = QBCore.Functions.GetPlayerData()
        QBCore.Functions.TriggerCallback('doom-spawn:server:getOwnedHouses', function(houses)
            local myHouses = {}
            if houses ~= nil then
                for i = 1, (#houses), 1 do
            ...
            if PlayerData.job.name == 'police' then 
                table.insert(spawns, QB.PoliceSpawn["missionrow"])
            end 

            if PlayerData.job.name == 'ambulance' then
                table.insert(spawns, QB.AmbulanceSpawn["pillbox"])
            end
